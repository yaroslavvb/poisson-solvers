...
9. **[Divide by Four and Conquer](https://yaroslavvb.github.io/poisson-solvers/reports/multigrid_explainer.html)** — geometric multigrid as an arrow-key walkthrough: nine live stages (why relaxation stalls, smoothing anatomy with the exact $\omega = 4/5 \Rightarrow$ factor-$3/5$ verification, smooth-error coarse-representability, full-weighting restriction, the $31{\to}15{\to}7{\to}3{\to}1$ cascade, the correction moment, post-smoothing, the assembled V with a walking marker, and the punchline table: $\rho_V = 0.187/0.190/0.191$ at $n = 15/31/63$ — mesh-independent within $0.004$ — vs Jacobi's $0.985/0.996/0.999$; V-PCG at $8/8/8$ iterations while plain CG climbs $35/74/149$), then the generalization: division of labor across scales, the $O(N)$ geometric-series work audit, Galerkin vs rediscretization, AMG and learned transfer operators — including the closing identity $\rho_{TG} = \mu^2 = 0.3576924$ that explains report 12's measured $0.357$ ([checks](python/experiments/multigrid_checks.py): 40 PASS).
