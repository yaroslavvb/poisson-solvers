...
The five racers: **plain gradient descent**, **CG**, **PCG + HODLR rank-2**, **PCG + HODLR rank-8**, and **damped Richardson + rank-8** ($\omega = 0.7376$, the sweep's optimum — the page notes that undamped Richardson diverges at rank 8, §4's finding 2). Controls: **Play** animates all enabled methods at the speed-slider rate (1–60 iterations/s) and auto-pauses when every method has converged; **Step** advances one iteration everywhere; **Reset** returns to $x_0 = 0$; per-method checkboxes hide or show a method's panel and error curve. Each panel renders the current temperature estimate $x_k$ on the $32\times32$ field with a fixed color scale taken from $x^\star$, so the hot spot (NW) and cold spot (SE) fade in at their true amplitudes; below, a shared log-scale chart tracks $\Vert x_k - x^\star\Vert/\Vert x^\star\Vert$ against the dashed $10^{-6}$ line.
