...
mathematica/
  ...
  prediction_tutorial.wls  # independent Wolfram companion of report 15: the n = 5 chain tutorial, all arithmetic exact rational
python/
  ...
  experiments/
    ...
    make_report13_diagrams.py, make_report13_anims.py  # report 13 schematics + GIFs (self-asserting) -> figures/diagram13_*, anim13_*
    hierarchical.py      # report 14 separator-rank checks + HODLR preconditioner sweep -> results/hierarchical.json + results/hodlr_viz_data.json + hierarchical_* figures + anim14_*
    make_report14_diagrams.py  # report 14 schematics (self-asserting) -> figures/diagram14_*
    prediction_tutorial.py # report 15 tutorial companion (exact-Fraction chain steps + 4x4-grid truncation + whitened-CG identity) -> results/prediction_tutorial.json + tutorial15_* figures
interactive/
  hierarchical-solvers.html  # report 14 in-browser solver race (loads results/hodlr_viz_data.json; serve repo root over HTTP)
  adi-sweep.html         # report 13 ADI half-sweep slider
reports/                 # this report suite (00..15)
results/                 # JSON summaries, npo_checkpoint.pt, npo_training_history.json, hodlr_viz_data.json (HODLR blocks for the report-14 browser demo)
...
uv run python python/experiments/grid_regressions_multiscale.py    # report 11 grid + multiscale checks (40 PASS)
...
uv run python python/experiments/prediction_tutorial.py            # report 15 tutorial companion (43 PASS, exact Fraction arithmetic)
...
wolframscript -file mathematica/prediction_tutorial.wls  # report 15 Wolfram cross-check (13 PASS, all arithmetic exact rational)
...
The reports are written to be read in numerical order; each is self-contained but cross-linked. One deliberate exception: **[15 — Preconditioning Is Approximate Prediction](15-preconditioning-as-prediction.md) is the suite's tutorial, and we suggest reading it *early*** — it replays the whole statistical arc of 09–14 as eight worked steps on the smallest possible examples ($n = 5$ chain in exact fractions, then a $4\times4$ grid), so it works as the on-ramp: read it right after 08 (or even first) to see the destination, then take 09–14 for the general treatments it points into.
...
**[14 — The Hierarchical Structure of the Inverse](14-hierarchical-inverse.md).** The structural sequel to 13 §3–4, joining three of the suite's facts into one: conditional independence across a separator ($\mathrm{Cov}(u_L, u_R \mid u_I) = 0$) *is* a rank bound on covariance blocks — $\Sigma_{LR} = \Sigma_{LI}\Sigma_{II}^{-1}\Sigma_{IR}$, rank $\le \vert I\vert$ (identity to $1.0\times10^{-15}$) — so every off-diagonal block of the dense $A^{-1}$ falls off a machine-precision cliff at exactly the separator width: 1 in 1-D (13's semiseparable $h\,x_i(1-x_j)$ triangle, re-derived as the one-node-blanket case), 32 at *every* level of a 2-D HODLR bisection ($s_{33}/s_1 \le 4.6\times10^{-16}$), rank ~11 for far blocks where geometric decay kicks in. That structure is then run as a solver: truncated-SVD HODLR copies $M_r$ of the inverse ($O(Nr\log N)$ storage, 8.0× compression at $r = 8$) swept over $r \in \{1..16\}$ as apply-ready SPD preconditioners — $\kappa(M_rA)$: 46.58 → 1.95, PCG 45 → 11 iterations, with three honest findings (block-Jacobi(2)'s 960-fold unit cluster still wins the iteration race until $r = 16$; undamped Richardson diverges for $r \le 8$ despite small $\kappa$; the build is pedagogical — assembled from the exact dense inverse). In 1-D the rank-1 HODLR inverse is exact and damped Richardson converges in one step. Comes with an [interactive in-browser solver race](../interactive/hierarchical-solvers.html) (§5) running GD, CG, PCG + the actual exported rank-2/rank-8 blocks, and damped Richardson live on the hot/cold-rod field (iterations to $10^{-6}$: 1190 / 54 / 24 / **10** / 21). Machine-checked by [hierarchical.py](../python/experiments/hierarchical.py) (29 checks, all quoted numbers in [results/hierarchical.json](../results/hierarchical.json)); schematics self-asserting in [make_report14_diagrams.py](../python/experiments/make_report14_diagrams.py).

**[15 — Preconditioning Is Approximate Prediction](15-preconditioning-as-prediction.md).** The suite run as a tutorial — and the recommended on-ramp (see the note at the top of this section): where 09–14 read as a treatise, 15 walks the same road in strict step-by-step form on examples small enough to print whole. Eight steps: the $n = 5$ Dirichlet chain in **exact rational arithmetic** — $A$ and its bridge inverse entry-by-entry (Step 1), the full-conditional predictor $B$, `D2` and the notebook identity `precisionMat == (I - B).D2` (Step 2), the sequential regressions `phiL2R`/`phiR2L` with coefficients $4/5, 3/4, 2/3, 1/2$ and exact innovation variances (Step 3), and perfect prediction solving $Ax = e_3$ in one predict-and-correct pass (Step 4) — then the $4\times4$ grid where truncation first bites: IC(0) as a truncated wavefront regression measured coefficient-by-coefficient against exact and Vecchia (19% of the row's $\ell_2$ mass dropped, Step 5), coarse regressors with variance explained $111/175$ exactly (Step 6), the measured Richardson tail matching $\rho(I - M^{-1}A)$ to five decimals for all three predictors (Step 7), and — the report's novel demonstration — the identity "PCG = CG in the predictor's whitened coordinates" made *executable*, trajectories coinciding to $3.9\times10^{-16}$ at every iterate for two structurally different preconditioners (Step 8). Notation follows the companion notebook `whitening_inverse_transposed.nb` throughout. Machine-checked by [prediction_tutorial.py](../python/experiments/prediction_tutorial.py) (43 checks, exact `Fraction` arithmetic for the chain steps; numbers in [results/prediction_tutorial.json](../results/prediction_tutorial.json)) and independently by [prediction_tutorial.wls](../mathematica/prediction_tutorial.wls) (13 checks, *all* arithmetic exact rational — no `N[]` anywhere).
