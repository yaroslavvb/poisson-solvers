...
|---|---:|---:|---:|---:|
| $\Vert \hat A_{\mathrm{rec}} - A\Vert _F/\Vert A\Vert _F$ | 0.882 | 0.146 | 0.0453 | 0.0143 |
...
2. **Sloppy whitener.** Keep only the stencil neighbors as regressors: incomplete Cholesky IC(0) $=$ the **Vecchia approximation** $=$ the KL-optimal factor for that sparsity pattern (Vecchia 1988; Schäfer–Katzfuss–Owhadi 2021; [09 §6](09-stiffness-as-precision.md)). ([11 §4](11-regressions-and-multiscale.md) dissects this rung on the 2-D grid: the dropped-fill mass, the measured IC(0)-vs-Vecchia coefficient gap, and the smooth error mode any local whitener leaves behind.)
